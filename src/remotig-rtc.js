...
const signalingUrl = 'wss://om4aa.ddns.net'
...
class RemotigRTCConnector {
  ...
  connect(tcvr, token, rig, successCallback, discCallback) {
    ...
    console.info('connecting ' + signalingUrl)
  ...
  disconnect(resetOrError = false) {
	...
	get connected() {
		return this._isStarted && this._pc && _cmdChannel
	}
  ...
  filter(bandWidth, centerFreq) {
    ...
    // }
    this.send('filter=' + bandWidth)
  }

  ////////////////////////////////////////////////////

  _connectSignaling() {
    this._signaling = io.connect(signalingUrl, signalingConfig)
...
_onCmdChannelOpen(event) {
  ...
  _bindCommands() {
    ...
    this.tcvr.bind(EventType.filter, RemotigRTCConnector.id, event => this.filter(event.value, tcvr.sidetoneFreq, port))
    ...
    this.tcvr.bind(EventType.resetAudio, RemotigRTCConnector.id, _ => this.restartAudio())
  }
